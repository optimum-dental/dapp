...
contract Escrow is Restrictor {
    ...
    inState(paymentId, 2)
  {
    ...
    uint scanApplicationId = DB(dbAddress).getUIntValue(keccak256("case/scan-application", caseId));
    ...
    userManager.releaseFundForScan(dbAddress, dentistId, patientId, scanApplicationId, amount, quote);
    FundReleased();
  }
    ...
    inState(paymentId, 2)
  {
    ...
    uint treatmentApplicationId = DB(dbAddress).getUIntValue(keccak256("treatment/treatment-application", treatmentId));
    ...
    DB(dbAddress).setUInt8Value(keccak256("payment/state", paymentId), 3);
    userManager.releaseFundForTreatment(dbAddress, dentistId, patientId, treatmentApplicationId, amount, quote);
    FundReleased();
  }
